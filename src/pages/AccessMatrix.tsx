...
import {
  ...
  Database, AlertTriangle, CheckSquare,
} from 'lucide-react';
      ...
      for (const db of ds.databases)
        for (const sch of db.schemas)
          for (const t of sch.tables)
            result.push({ id: t.id, name: t.name, path: `${db.name} › ${sch.name}`, system: ds.name, systemType: ds.type, resourceType: 'table' });
    }
    if (ds.workspaces) {
      for (const ws of ds.workspaces)
        for (const rpt of ws.reports)
          result.push({ id: rpt.id, name: rpt.name, path: ws.name, system: ds.name, systemType: ds.type, resourceType: 'report' });
    }
  }
  return result;
}
...
// ─── Revoke Modal (single & bulk) ─────────────────────────────────────────────

interface RevokeModalProps {
  targets:   RevokeTarget[];      // 1 = single, N = bulk
  user:      User;
  onClose:   () => void;
...
const RevokeModal: React.FC<RevokeModalProps> = ({ targets, user, onClose, onConfirm, loading }) => {
  ...
  return (
    <Modal
      open
      onClose={onClose}
      title={isBulk ? `Request Removal — ${targets.length} Resources` : 'Request Access Removal'}
      size="md"
      footer={
        <>
          ...
          >
            {isBulk ? `Submit ${targets.length} Removal Requests` : 'Submit Removal Request'}
          </Button>
        </>
      }
    >
      <div className="space-y-4">
        ...
        <div className="flex items-start gap-3 bg-red-50 border border-red-200 rounded-xl p-4">
          ...
          <div>
            <p className="text-sm font-semibold text-red-700">
              {isBulk ? `Remove access to ${targets.length} resources` : 'Remove Access Request'}
            </p>
            <p className="text-xs text-red-600 mt-0.5">
              Access will only be revoked after admin approval.
            </p>
          </div>
        </div>
        ...
        <div className="bg-gray-50 rounded-xl p-4 space-y-3 text-sm">
          ...
          {isBulk ? (
            /* Bulk: scrollable resource list */
            <div>
              <p className="text-gray-500 mb-2">Resources to revoke</p>
              <div className="space-y-1.5 max-h-44 overflow-y-auto pr-1">
                {targets.map((t) => (
                  <div key={t.resourceId} className="flex items-center justify-between bg-white border border-gray-200 rounded-lg px-3 py-2">
                    <div className="flex items-center gap-2 min-w-0">
                      <span className="text-sm flex-shrink-0">{t.resourceType === 'table' ? '🗄️' : '📊'}</span>
                      <div className="min-w-0">
                        <p className="font-medium text-gray-800 truncate text-xs">{t.resourceName}</p>
                        <p className="text-xs text-gray-400 truncate">{t.resourcePath}</p>
                      </div>
                    </div>
                    <div className="flex gap-1 flex-shrink-0 ml-2">
                      {t.permissions.map((p) => {
                        const cls = { read: 'bg-blue-100 text-blue-700', write: 'bg-purple-100 text-purple-700', admin: 'bg-red-100 text-red-700' }[p];
                        return <span key={p} className={`px-1.5 py-0.5 rounded text-xs font-semibold capitalize ${cls}`}>{p}</span>;
                      })}
                    </div>
                  </div>
                ))}
            ...
            <>
              <div className="flex justify-between items-center">
                ...
                <div className="text-right">
                  <p className="font-medium text-gray-800">{targets[0].resourceName}</p>
                  <p className="text-xs text-gray-400">{targets[0].resourcePath}</p>
                </div>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-gray-500">System</span>
                <SystemBadge type={targets[0].systemType} />
              </div>
              <div className="flex justify-between items-center">
                <span className="text-gray-500">Current Permissions</span>
                <div className="flex gap-1">
                  {targets[0].permissions.map((p) => {
                    const cls = { read: 'bg-blue-100 text-blue-700', write: 'bg-purple-100 text-purple-700', admin: 'bg-red-100 text-red-700' }[p];
                    return <span key={p} className={`px-2 py-0.5 rounded text-xs font-semibold capitalize ${cls}`}>{p}</span>;
                  })}
                </div>
              </div>
            </>
...
const PermChip: React.FC<{
  permission: Permission; active: boolean; readonly: boolean; onToggle: () => void;
}> = ({ permission, active, readonly, onToggle }) => {
...
const UserDetailPanel: React.FC<{ user: User; isAdmin: boolean }> = ({ user, isAdmin }) => {
  ...
  const [revokeTargets, setRevokeTargets] = useState<RevokeTarget[] | null>(null);
  ...
  const rows = useMemo(() => {
    return entries
      .map((e) => { const meta = CATALOG.find((c) => c.id === e.resourceId); return meta ? { entry: e, meta } : null; })
  ...
  // Reset selection when user or filter changes
  useMemo(() => { setSelectedIds(new Set()); }, [user.id, systemFilter]);
  ...
  const SYSTEM_FILTERS = [
    ...
    { value: 'powerbi' as const,    label: 'Power BI'   },
  ];

  const allVisibleIds  = rows.map((r) => r.entry.resourceId);
  ...
  function buildTargets(ids: string[]): RevokeTarget[] {
    return ids.map((id) => {
      const row = rows.find((r) => r.entry.resourceId === id)!;
      return {
        resourceId:   row.entry.resourceId,
        resourceName: row.meta.name,
        resourcePath: row.meta.path,
        systemType:   row.meta.systemType,
        resourceType: row.meta.resourceType,
        permissions:  row.entry.permissions,
      };
    });
  }

  async function handleRevokeConfirm(comment: string) {
    if (!revokeTargets) return;
    setRevokeLoading(true);
    await new Promise((r) => setTimeout(r, 900));

    if (revokeTargets.length === 1) {
      const t = revokeTargets[0];
      submitRequest({
      ...
      // One combined revoke request with all resourceIds
      submitRequest({
        requesterId:   currentUser.id,
        targetUserId:  user.id,
        systemType:    revokeTargets[0].systemType,
        resourceId:    revokeTargets[0].resourceId,
        resourceName:  revokeTargets.map((t) => t.resourceName).join(', '),
        resourcePath:  revokeTargets[0].resourcePath,
        resourceIds:   revokeTargets.map((t) => t.resourceId),
        resourceNames: revokeTargets.map((t) => t.resourceName),
        accessLevel:   'none',
        reason:        comment,
        requestType:   'revoke',
      });
  ...
  return (
    <div className="flex flex-col h-full">
        ...
        {SYSTEM_FILTERS.map(({ value, label }) => (
          <button
            ...
            className={`px-3 py-1.5 text-xs font-medium rounded-lg whitespace-nowrap transition-colors ${
              systemFilter === value ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'
            }`}
          >
            {label}
          </button>
      ...
      {isAdmin && someSelected && (
        <div className="flex items-center justify-between px-6 py-2.5 bg-red-50 border-b border-red-200 flex-shrink-0">
          <div className="flex items-center gap-2">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button
              ...
              onClick={() => setRevokeTargets(buildTargets([...selectedIds]))}
            >
              Revoke Selected
            </Button>
          </div>
      ...
      <div className="flex-1 overflow-y-auto scrollbar-thin">
        {rows.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full py-16 text-center">
            ...
            <p className="text-xs text-gray-400 mt-1">This user has no access for the selected filter.</p>
          </div>
        ) : (
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-50">
              {rows.map(({ entry, meta }) => {
                const isChecked = selectedIds.has(entry.resourceId);
                return (
                  <tr
                    key={entry.resourceId}
                    className={`hover:bg-gray-50/60 group transition-colors ${isChecked ? 'bg-red-50/40' : ''}`}
                  >
                    {/* Checkbox */}
                    {isAdmin && (
                      <td className="px-4 py-3">
                        <button
                          onClick={() => toggleRow(entry.resourceId)}
                          className="flex items-center justify-center"
                        >
                          <div className={`w-4 h-4 rounded border-2 flex items-center justify-center transition-colors ${
                            isChecked ? 'bg-red-500 border-red-500' : 'border-gray-300 hover:border-red-400'
                          }`}>
                            {isChecked && (
                              <svg className="w-2.5 h-2.5 text-white" viewBox="0 0 10 10" fill="none">
                                <path d="M1.5 5L4 7.5L8.5 2.5" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" />
                    ...
                    {/* Single revoke button */}
                    {isAdmin && (
                      <td className="px-4 py-3">
                        <button
                          onClick={() => setRevokeTargets([{
...
export const AccessMatrix: React.FC = () => {
  ...
  function countPermissions(userId: string) {
    ...
    return { resources: entries.length, permissions: entries.reduce((s, e) => s + e.permissions.length, 0) };
  }
    ...
    <div className="flex flex-col gap-4">
      <div>
        ...
        <p className="text-sm text-gray-500 mt-1">
          Select a user to view and manage their resource permissions.
          {isAdmin && <span className="ml-2 text-blue-600">Check rows to bulk-revoke; click chips to toggle individually.</span>}
        </p>
      </div>
      ...
      <div className="flex gap-4 min-h-[calc(100vh-200px)]">
        ...
        <Card padding={false} className="w-72 flex-shrink-0 flex flex-col h-full overflow-hidden">
              ...
              return (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <p className={`text-sm font-medium truncate ${isSelected ? 'text-blue-700' : 'text-gray-800'}`}>{user.name}</p>
          ...
          <div className="px-4 py-2 border-t border-gray-100 text-xs text-gray-400 flex gap-3 flex-shrink-0">
            <span>r = resources</span><span>p = permissions</span>
          </div>
        </Card>
          ...
          {selectedUser ? (
            ...
            <div className="flex flex-col items-center justify-center h-full py-24 text-center">
              ...
              <p className="text-sm text-gray-400 max-w-xs">
                Choose a user from the list on the left to see and manage their resource permissions.
              </p>
              <div className="mt-6 flex items-center gap-4 text-xs text-gray-400">
                <span className="flex items-center gap-1.5"><span className="w-3 h-3 rounded bg-blue-500 inline-block" /> Read</span>
                <span className="flex items-center gap-1.5"><span className="w-3 h-3 rounded bg-purple-500 inline-block" /> Write</span>
                <span className="flex items-center gap-1.5"><span className="w-3 h-3 rounded bg-red-500 inline-block" /> Admin</span>
                <span className="flex items-center gap-1.5"><Database className="w-3 h-3" /> Table</span>
              </div>
            </div>
          )}
        </Card>
